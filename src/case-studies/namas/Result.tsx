...
/**
 * Maps scroll progress (0→1 from section entering to section **center** at viewport center)
 * onto the clip reveal. Ends at `1` so the animation finishes when Result is centered.
 */
...
export type NamasResultProps = {
  scrollContainerRef: RefObject<HTMLElement | null>;
};

export function NamasResult({ scrollContainerRef }: NamasResultProps) {
  const sectionRef = useRef<HTMLElement>(null);
  const { scrollYProgress } = useScroll({
    container: scrollContainerRef,
    target: sectionRef,
    /** 0 = section entering; 1 = section center aligned with scrollport center — reveal completes here */
      ...
      <div className="mx-auto max-w-5xl">
        <div className="flex items-stretch gap-0">
          ...
          <ResultCardReveal scrollYProgress={scrollYProgress}>
            ...
            >
              Enabled dynamic project updates and improved service visibility for
              potential clients
            </motion.p>
          </ResultCardReveal>
        </div>
