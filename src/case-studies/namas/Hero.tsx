...
const titleLineVariants = {
  ...
  show: (i: number) => ({
    ...
    y: 0,
    skewY: 0,
    transition: { duration: 0.85, ease: easeOut, delay: 0.35 + i * 0.12 },
  }),
} as const;

export function NamasHero() {
  const reduce = useReducedMotion();

  return (
    <section className="relative flex min-h-screen flex-col overflow-hidden bg-black px-6 pt-24 md:px-10">
      <motion.div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,rgba(255,255,255,0.08),transparent_55%)]"
      ...
      >
          ...
          >
            <motion.span
              ...
              animate={
                reduce
                  ? undefined
                  : { boxShadow: ['0 0 0 0 rgba(255,255,255,0)', '0 0 0 8px rgba(255,255,255,0.06)', '0 0 0 0 rgba(255,255,255,0)'] }
              }
        ...
        <motion.div
          ...
          initial={reduce ? false : undefined}
          animate={reduce ? { opacity: 1 } : undefined}
          className="min-w-0 max-w-2xl flex-1 pt-0.5 md:pt-[40px]"
        >
          <motion.h1
      ...
      <div className="relative z-10 flex flex-1 flex-col pb-16">
        <div className="mt-auto flex items-end justify-between gap-6">
          ...
          <div className="text-right">
            <motion.h2
              className="font-serif text-6xl font-light leading-[0.95] tracking-tight md:text-8xl lg:text-9xl"
              initial={reduce ? false : { opacity: 0 }}
              animate={{ opacity: 1 }}
            >
              {reduce ? (
                <>
                  <span className="block">Namas Design</span>
                  <span className="block italic text-white/95">and Build</span>
                </>
              ) : (
                <>
                  <motion.span
                  ...
                  <motion.span
                    ...
                    animate="show"
                  >
                    <span className="block text-white/95">and Build</span>
                  </motion.span>
                </>
              )}
            </motion.h2>
