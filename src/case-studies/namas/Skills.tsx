...
function SkillLine({
  ...
  const letterSpacing = useTransform(
    ...
    index === 0
      ? [0, 1 / n, 1]
      : [0, index / n, (index + 1) / n, 1],
    index === 0 ? ['0.02em', '0.12em', '0.12em'] : ['0.02em', '0.02em', '0.1em', '0.1em']
  );
...
export type NamasSkillsProps = {
  /** Same scroll container as the case-study `<main>` (not the window). */
  scrollContainerRef: RefObject<HTMLElement | null>;
};

export function NamasSkills({ scrollContainerRef }: NamasSkillsProps) {
  ...
  return (
    <section ref={sectionRef} className="relative overflow-hidden bg-black px-6 py-32 text-white">
      <motion.div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_70%_50%_at_50%_50%,rgba(255,255,255,0.08),transparent)]"
